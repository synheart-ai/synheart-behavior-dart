...
### Docs
- README event-type list and `BehavioralMetrics` field names corrected (`behavioralDistractionScore`, `idleTimeRatio`).

## [0.2.1] - 2026-02-18

### Changed

- Removed Flux vendoring/integration artifacts (native bridges and bundled binaries) so this package ships as a standard Flutter plugin without bundled Flux binaries.
- Behavioral/session metrics are computed locally by the native iOS/Android implementations.

### Fixed

- Removed an unused Dart dependency (`ffi`).
