...
class SynheartBehaviorPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            "initialize" -> {
                val config = call.arguments as? Map<*, *> ?: emptyMap()
                initialize(config)
                result.success(null)
            }
            "startSession" -> {
                val args = call.arguments as? Map<*, *> ?: emptyMap()
    ...
    private fun initialize(config: Map<*, *>) {
        val behaviorConfig = BehaviorConfig(
            enableInputSignals = config["enableInputSignals"] as? Boolean ?: true,
            enableAttentionSignals = config["enableAttentionSignals"] as? Boolean ?: true,
        ...
        behaviorSDK?.setEventHandler { event ->
            emitEvent(event.toMap())
        }
    }
    ...
    private fun getCurrentStats(): Map<String, Any?> {
        return behaviorSDK?.getCurrentStats()?.toMap() ?: mapOf(
    ...
    private fun endSession(sessionId: String): Map<String, Any?> {
        return try {
            behaviorSDK?.endSession(sessionId)?.toMap() ?: mapOf(
    ...
    private fun updateConfig(config: Map<*, *>) {
        val behaviorConfig = BehaviorConfig(
            enableInputSignals = config["enableInputSignals"] as? Boolean ?: true,
            enableAttentionSignals = config["enableAttentionSignals"] as? Boolean ?: true,
            enableMotionLite = config["enableMotionLite"] as? Boolean ?: false,
